...
function validateStatus(status) {
  ...
  const aliases = {
    // English canonicals
    'evaluated': 'Evaluated', 'applied': 'Applied', 'responded': 'Responded', 'interview': 'Interview', 'offer': 'Offer', 'rejected': 'Rejected', 'discarded': 'Discarded', 'skip': 'SKIP',
    'sent': 'Applied', 'hold': 'Evaluated', 'closed': 'Discarded', 'cancelled': 'Discarded', 'monitor': 'SKIP', 'geo blocker': 'SKIP',
  };
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const col4LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|sent|hold|closed|cancelled|monitor)/i.test(col4);
    const col5LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|sent|hold|closed|cancelled|monitor)/i.test(col5);
