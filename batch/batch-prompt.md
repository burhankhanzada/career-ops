...
3. Tracker line for subsequent merge
...
**RULE: For article metrics, article-digest.md prevails over cv.md.** cv.md might have older numbers — this is normal.
...
2. If the file is empty or does not exist, attempt to fetch the JD from `{{URL}}` using WebFetch
...
**Transversal Advantage**: Frame profile as a **"Technical builder"** that adapts framing to the role:
...
Turn "builder" into a professional signal, not a "hobby maker." The framing changes, the truth remains the same.
...
- FDE → prioritize fast delivery and client-facing skills
...
Use WebSearch for current salaries (Glassdoor, Levels.fyi, Blind), company comp reputation, demand trends. Table with data and cited sources. If there is no data, say so.
...
7. Select top 3-4 projects most relevant
8. Reorder experience bullets by JD relevance
9. Build competency grid (6-8 keyword phrases)
10. Inject keywords into existing achievements (**NEVER invent them**)
...
**Valid canonical states:** `Evaluada`, `Applied`, `Responded`, `Interview`, `Offer`, `Rejected`, `Discarded`, `NO APLICAR`
