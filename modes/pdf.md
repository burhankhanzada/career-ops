...
6. Detect role archetype → adjust framing.
7. Rewrite Professional Summary injecting JD keywords + exit narrative bridge (e.g., "Built and sold a business. Now applying systems thinking to [JD domain].").
...
11. Inject keywords naturally into existing achievements (NEVER invent them).
12. Generate full HTML from template + personalized content. (Template handles `**bold**` automatically via JS).
...
16. Report: PDF path, page count, keyword coverage percentage.
...
- Inject keywords naturally (NEVER invent them).
