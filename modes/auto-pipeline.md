...
**If the input is JD text** (not a URL): use it directly, without a fetch.

## Step 1 — Evaluation A-G
Execute exactly the same as the `offer` mode (read `modes/oferta.md` for all blocks A-F + Block G Posting Legitimacy).
...
Register in `data/applications.md` with all columns including Report and PDF checked ✅.
